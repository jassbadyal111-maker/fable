...
MODELS = [m.strip() for m in os.getenv("FABLE_MODELS", "claude-fable-5").split(",") if m.strip()]
DEFAULT_MODEL = os.getenv("FABLE_DEFAULT_MODEL", MODELS[0])
...
SYSTEM_PROMPT = "You are Fable, a helpful AI assistant. Be direct, accurate, and conversational."
...
def create_chat(model=DEFAULT_MODEL):
    ...
    with closing(db()) as conn:
        conn.execute(
            ...
            (chat_id, "New chat", model, timestamp, timestamp),
        )
        conn.commit()
    return chat_id
...
async def stream_message(chat_id: str, payload: MessageCreate, request: Request):
    ...
    with closing(db()) as conn:
        ...
        conn.execute(
            ...
            (chat_id, "user", content, timestamp),
        )
        if count == 0:
            title = content.replace("\n", " ").strip()[:45] or "New chat"
            conn.execute("UPDATE chats SET title=?,model=?,updated_at=? WHERE id=?", (title, model, timestamp, chat_id))
        else:
            conn.execute("UPDATE chats SET model=?,updated_at=? WHERE id=?", (model, timestamp, chat_id))
        conn.commit()
